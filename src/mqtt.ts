...
export class MQTT {
    ...
    constructor(
        ...
        if (!this.username || !this.password || !this.hostname) {
            console.log("MQTT: ", "You must set a mqtt username and password in the addon config")
            process.exit(22);
        }
    }
        ...
        return new Promise((resolve, reject) => {
            if (!this.client) {
                ...
                this.client.on("connect", (packet) => {
                    console.log("MQTT", `Connected ${this.client_id}`);
                    // console.log("MQTT", `${JSON.stringify(packet)}`);
                    if (!resolved) {
                        resolve(this.client);
                    } else {
                        console.log("MQTT", `Reconnected ${this.client_id}`);
                    }
                });
            } else {
                resolve(this.client);
    ...
    public async stop(force: boolean) {
        return new Promise((resolve, reject) => {
            this.client.end(force, () => {
                console.log("MQTT", `Disconnected ${this.client_id}`);
    ...
    public async publish(topic: string, message: string, settings: IClientPublishOptions) {
        ...
        return new Promise((resolve, reject) => {
            this.client.publish(topic, message, settings, (e, p) => {
                if (e) {
                    console.log("MQTT ERROR:", `${e.toString()}`);
                    reject(e);
                } else {
                    resolve(p);
                }
